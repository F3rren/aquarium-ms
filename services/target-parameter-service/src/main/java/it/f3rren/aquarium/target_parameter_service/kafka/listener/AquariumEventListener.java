...
public class AquariumEventListener {
    ...
    public void onAquariumEvent(AquariumEvent event) {
        if ("DELETED".equals(event.type())) {
            log.info("Received DELETED event for aquarium ID: {} — removing target parameters", event.aquariumId());
            targetParameterRepository.deleteAllByAquariumId(event.aquariumId());
        }
    }
}
